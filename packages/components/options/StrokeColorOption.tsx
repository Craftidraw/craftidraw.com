...
const StrokeColorOption: React.FC<StrokeColorOptionProps> = ({ item }) => {
    ...
    useEffect(() => {
        if (!isLocalUpdate.current) {
            let rgba;
            if (item.strokeColor) rgba = rgbToHex(item.strokeColor);
            if (rgba) {
                setColor(rgba.color);
    ...
    const debounceColorChange = useCallback(
        debounce((item: Item, color: string, opacity: number) => {
            updateItem({ ...item, strokeColor: hexToRgbA(color, opacity) }, item);
        }, 500),
        [],
    );
        ...
        <>
            {item.isStrokeable && item.isStokeEnabled && (
                <>
                    ...
                    <Form.Label style={{ marginBottom: '0' }}>Stroke Color Opacity</Form.Label>
